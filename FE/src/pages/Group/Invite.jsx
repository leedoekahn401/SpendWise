import { Spin, List, Button, Avatar, Empty, notification } from 'antd';
...
const InvitesPage = () => {
    ...
    const fetchInvites = async () => {
        try {
            ...
            notification.error({ message: "Cant load invitation list" });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleAction = async (inviteId, action) => {
        try {
            if (action === "accept") {
                ...
                notification.info({ message: "Declined" });
            }
            ...
            {invites.length === 0 ? (
                ...
                <List
                    ...
                    renderItem={(invite) => (
                        ...
                        >
                            <List.Item.Meta
                                avatar={<Avatar src={invite.inviter.profilePic} size={40} />}
                                title={
                                <span>{invite.group.name}</span>

                                }
                                description={<span>Sent by {invite.inviter.username} at {new Date(invite.createdAt).toLocaleString()}</span>}
                            />
                        </List.Item>
                    )}
                />
